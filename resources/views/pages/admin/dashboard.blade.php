    ...
    <div class="container-fluid py-4">
                ...
                <div class="card-body row g-3 align-items-center">
                    <div class="col-12 col-md-6">
                        <div class="input-group">
                            ...
                            <input type="text" name="search" class="form-control border-start-0" placeholder="Search business name or DOS ID..." value="{{ request('search') }}">
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-2">
                        <select name="paid" class="form-select">
                            ...
                            <option value="Unpaid" {{ request('paid') == 'Unpaid' ? 'selected' : '' }}>Unpaid</option>
                        </select>
                    </div>
                    <div class="col-12 col-md-2">
        ...
        <div class="d-flex justify-content-end mb-3">
            <form method="POST" action="{{ route('admin.submissions.bulk-mail') }}" id="bulkMailForm">
                ...
                <button type="submit" class="btn btn-info" id="bulkMailBtn" disabled>Send Mail to Selected (<span id="selectedCount">0</span>)</button>
            </form>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><input type="checkbox" name="submission_ids[]" value="{{ $submission->id }}" class="row-checkbox"></td>
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="avatar me-3" style="width: 40px; height: 40px; background-color: #e9ecef; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold;">
                            ...
                            <td>
                                <span class="badge bg-{{ $submission->is_paid ? 'paid' : 'unpaid' }}">{{ $submission->is_paid ? 'Paid' : 'Unpaid' }}</span>
                            </td>
                            <td>
                                <div class="d-flex flex-wrap gap-2">
                                    <a href="{{ route('admin.submissions.show', $submission->id) }}" class="btn btn-sm btn-outline-primary" title="View"><i class="bi bi-eye"></i></a>
                                    <a href="{{ route('submission.pdf', $submission->id) }}" class="btn btn-sm btn-outline-success" title="Download PDF"><i class="bi bi-file-earmark-pdf"></i></a>

                                    <form class="action-form" method="POST" action="{{ route('admin.submission.send-mail', $submission->id) }}">
                                        @csrf
                                        <button type="submit" class="btn btn-sm btn-outline-warning" title="Send Email"><i class="bi bi-envelope"></i></button>
                                    </form>

                                    <button type="button" class="btn btn-sm btn-outline-secondary" title="Update"><i class="bi bi-pencil-square"></i></button>
                                </div>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="6" class="text-center py-4">No submissions found matching your criteria.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        updateSelectionState(); // Initial check on page load
    });
</script>
@endsection