...
<script>
const mockData = [
    { id: 1, businessName: "Tech Solutions Inc.", dosId: "DOS-2024-001", status: "Approved", isPaid: true, email: "contact@techsolutions.com", trend: "up" },
    { id: 2, businessName: "Green Energy LLC", dosId: "DOS-2024-002", status: "Under Review", isPaid: false, email: "info@greenenergy.com", trend: "down" },
    { id: 3, businessName: "Marketing Pro Agency", dosId: "DOS-2024-003", status: "Pending", isPaid: true, email: "hello@marketingpro.com", trend: "up" },
    { id: 4, businessName: "Food Delivery Express", dosId: "DOS-2024-004", status: "Rejected", isPaid: false, email: "support@fooddelivery.com", trend: "down" },
    { id: 5, businessName: "Construction Masters", dosId: "DOS-2024-005", status: "Approved", isPaid: true, email: "office@constructionmasters.com", trend: "up" }
];
...
function populateTable() {
    ...
    const filtered = mockData.filter(item => {
        const matchSearch = item.businessName.toLowerCase().includes(search) || item.dosId.toLowerCase().includes(search);
        const matchStatus = status === "All" || item.status === status;
        const matchPaid = paid === "All" || (paid === "Paid" && item.isPaid) || (paid === "Unpaid" && !item.isPaid);
    ...
    filtered.forEach(item => {
        ...
        <tr>
            ...
            <td><span class="badge bg-${item.isPaid ? 'success' : 'danger'}">${item.isPaid ? 'Paid' : 'Unpaid'}</span></td>
            <td>
                <div class="d-flex flex-wrap gap-2">
                    <button class="btn btn-sm btn-outline-primary" title="View"><i class="bi bi-eye"></i></button>
                    <button class="btn btn-sm btn-outline-success" title="Download PDF"><i class="bi bi-file-earmark-pdf"></i></button>
                    <button class="btn btn-sm btn-outline-warning" title="Send Email"><i class="bi bi-envelope"></i></button>
                    <button class="btn btn-sm btn-outline-secondary" title="Update"><i class="bi bi-pencil-square"></i></button>
                </div>
            </td>
        </tr>`;
    });
